...
if($action == 'env') {
	...
	if(version_compare(PHP_VERSION, '5.3.0') == -1) {
		...
		$ret['php']['version']['remark'] = 'PHP版本必须为 5.3.0 以上. <a href="http://www.heitoy.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58062">详情</a>';
	}
	...
	if($ret['php']['pdo']['ok']) {
		...
		if(!$ret['php']['mysql']['ok']) {
			$ret['php']['pdo']['remark'] = '您的PHP环境不支持 mysql_connect，请开启此扩展. <a target="_blank" href="http://www.heitoy.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58073">详情</a>';
		}
	} else {
		$ret['php']['pdo']['failed'] = true;
		if($ret['php']['mysql']['ok']) {
			...
			$ret['php']['pdo']['remark'] = '您的PHP环境不支持PDO, 请开启此扩展. <a target="_blank" href="http://www.heitoy.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58074">详情</a>';
		} else {
			$ret['php']['pdo']['value'] = '<span class="glyphicon glyphicon-remove text-danger"></span>';
			$ret['php']['pdo']['class'] = 'danger';
			$ret['php']['pdo']['remark'] = '您的PHP环境不支持PDO, 也不支持 mysql_connect, 系统无法正常运行. <a target="_blank" href="http://www.heitoy.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58074">详情</a>';
		}
	}
	...
	if($ret['php']['curl']['ok']) {
		...
		if(!$ret['php']['fopen']['ok']) {
			$ret['php']['curl']['remark'] = '您的PHP环境虽然不支持 allow_url_fopen, 但已经支持了cURL, 这样系统是可以正常高效运行的, 不需要额外处理. <a target="_blank" href="http://www.heitoy.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58076">详情</a>';
		}
	} else {
		if($ret['php']['fopen']['ok']) {
			$ret['php']['curl']['value'] = '<span class="glyphicon glyphicon-remove text-warning"></span>';
			$ret['php']['curl']['class'] = 'warning';
			$ret['php']['curl']['remark'] = '您的PHP环境不支持cURL, 但支持 allow_url_fopen, 这样系统虽然可以运行, 但还是建议你开启cURL以提升程序性能和系统稳定性. <a target="_blank" href="http://www.heitoy.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58086">详情</a>';
		} else {
			$ret['php']['curl']['value'] = '<span class="glyphicon glyphicon-remove text-danger"></span>';
			$ret['php']['curl']['class'] = 'danger';
			$ret['php']['curl']['remark'] = '您的PHP环境不支持cURL, 也不支持 allow_url_fopen, 系统无法正常运行. <a target="_blank" href="http://www.heitoy.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58086">详情</a>';
	...
	if($ret['php']['ssl']['ok']) {
		...
		$ret['php']['ssl']['remark'] = '没有启用OpenSSL, 将无法访问公众平台的接口, 系统无法正常运行. <a target="_blank" href="http://www.heitoy.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58109">详情</a>';
	}
	...
	if($ret['php']['gd']['ok']) {
		...
		$ret['php']['gd']['remark'] = '没有启用GD, 将无法正常上传和压缩图片, 系统无法正常运行. <a target="_blank" href="http://www.heitoy.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58110">详情</a>';
	}
	...
	if($ret['php']['dom']['ok']) {
		...
		$ret['php']['dom']['remark'] = '没有启用DOMDocument, 将无法正常安装使用模块, 系统无法正常运行. <a target="_blank" href="http://www.heitoy.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58111">详情</a>';
	}
	...
	if($ret['php']['session']['ok'] == 0 || strtolower($ret['php']['session']['ok']) == 'off') {
		...
		$ret['php']['session']['remark'] = '系统session.auto_start开启, 将无法正常注册会员, 系统无法正常运行. <a target="_blank" href="http://www.heitoy.com/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58111">详情</a>';
	}
	...
	if($ret['write']['root']['ok']) {
		...
		$ret['write']['root']['remark'] = '本地目录无法写入, 将无法使用自动更新功能, 系统无法正常运行.  <a href="http://www.heitoy.com/">详情</a>';
	}
	...
	if($ispost) {
		...
		if(empty($error)) {
			...
			if($_POST['type'] == 'remote') {
				...
				if($ins['error']) {
					die('<script type="text/javascript">alert("链接moyang更新服务器失败, 错误为: ' . $ins['error'] . '！");history.back();</script>');
				}
				$archive = $ins['files'];
				if(!$archive) {
...
VER;
			...
			if(file_exists(IA_ROOT . '/index.php') && is_dir(IA_ROOT . '/web') && file_exists($verfile) && file_exists($dbfile)) {
				...
				die('<script type="text/javascript">alert("你正在使用本地安装, 但未下载完整安装包, 请从moyang官网下载完整安装包后重试.");history.back();</script>');
			}
...
<html lang="zh-cn">
	<head>
		...
		<title>安装系统 - moyang - 公众平台自助开源引擎</title>
		...
		<div class="container">
			<div class="header" style="margin:15px auto;">
				<ul class="nav nav-pills pull-right" role="tablist">
					...
					<li role="presentation"><a href="http://www.heitoy.com">moyang官网</a></li>
					<li role="presentation"><a href="http://www.heitoy.com">访问论坛</a></li>
				</ul>
				<img src="?res=logo" />
			</div>
			...
			<div class="footer" style="margin:15px auto;">
				<div class="text-center">
					<a href="http://www.heitoy.com">关于</a> &nbsp; &nbsp; <a href="http://www.heitoy.com">帮助</a> &nbsp; &nbsp; <a href="http://www.heitoy.com">购买授权</a>
				</div>
				<div class="text-center">
					Powered by <a href="http://www.heitoy.com"><b>moyang</b></a> v0.7 &copy; 2014 <a href="http://www.heitoy.com">www.heitoy.com</a>
				</div>
			</div>
		</div>
	...
	echo <<<EOF
		<div class="panel panel-default">
			...
			<div class="panel-body" style="overflow-y:scroll;max-height:400px;line-height:20px;">
				<h3>版权所有 (c)2014，团队保留所有权利。 </h3>
				<p>
					感谢您选择moyang - 微信公众平台自助开源引擎（以下简称WE7，WE7基于 PHP + MySQL的技术开发，全部源码开放。 <br />
					为了使你正确并合法的使用本软件，请你在使用前务必阅读清楚下面的协议条款：
				</p>
				<p>
					<strong>一、本授权协议适用且仅适用于系统(We7, MicroEngine. 以下简称moyang)任何版本，moyang官方对本授权协议的最终解释权。</strong>
				</p>
				<p>
					<strong>二、协议许可的权利 </strong>
					<ol>
						<li>您可以在完全遵守本最终用户授权协议的基础上，将本软件应用于非商业用途，而不必支付软件版权授权费用。</li>
						<li>您可以在协议规定的约束和限制范围内修改moyang源代码或界面风格以适应您的网站要求。</li>
				...
				<p>
					...
					<ol>
						...
						<li>未经官方许可，禁止在moyang的整体或任何部分基础上以发展任何派生版本、修改版本或第三方版本用于重新分发。</li>
...
function tpl_install_env($ret = array()) {
	...
	echo <<<EOF
		...
		<div class="alert alert-info">系统要求moyang整个安装目录必须可写, 才能使用moyang所有功能。</div>
		<div class="panel panel-default">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						...
						<div class="help-block">
							...
							在线安装能够直接安装最新版本系统, 如果在线安装困难, 请下载离线安装包后使用本地安装. <br/>
...
function tpl_install_finish() {
	...
	<div class="alert alert-success">
		恭喜您!已成功安装“moyang - 公众平台自助开源引擎”系统，您现在可以: <a target="_blank" class="btn btn-success" href="./web/index.php">访问网站首页</a>
	</div>
	<div class="form-group">
		<h5><strong>moyang应用商城</strong></h5>
